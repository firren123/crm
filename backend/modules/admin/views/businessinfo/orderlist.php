            ...
            <?php if(empty($data)) {
                ...
                foreach ($data as $item):
                    ...
                    <tr>
                        ...
                        <td class="shop_id"><?= $item['shop_id'];?></td>
                        <td><?=$item['order_total']?></td>
                        <td><?= $item['money_total'];?></td>
                        <td><?= $item['zy_money_total'];?></td>
                        <td><?= $item['tg_money_total'];?></td>
                        <td><a href="javascript:;" class="shop_view">详情</a></td>
                    </tr>
                <?php endforeach;
            }
            ?>
...
<script type="text/javascript">
    $(function(){
        $(".shop_view").click(function(){
            var id = $(this).parent().closest("tr").find(".shop_id").text();
            var d = dialog({
                url:'/admin/businessinfo/shop-orderview?id='+id,
                title: '商家订单流水详情',
                width: '50em',
                okValue: '确定',
                lock: true,
