...
<?= $form->field($AppVersionList_model, 'is_forced_to_update')->label('是否强制更新：',['style'=>'width:180px'])
    ->radioList(['0'=>'不强制更新','1'=>'强制更新'],['onchange'=>'checkupdatestatus(this)'],['style'=>'float: left;']) ;?>
<?= $form->field($AppVersionList_model, 'update_prompt')->label('更新提示时间：',['style'=>'width:180px'])
    ->input('text', ['style'=>'width:200px', 'maxlength'=>'10', 'onchange'=>'checkupdate_prompt(this)', 'onfocus'=>"WdatePicker({isShowClear:true, readOnly:false})"]) ; ?>
...
<script language="javascript">
    ...
    window.onload = function(){
        ...
        if(document.getElementById('appversionlist-is_forced_to_update').value!=''){update_status = true;}
        if(document.getElementById('appversionlist-update_prompt').value!=''){update_prompt = true;}
        ...
        if(file.value!=''){
            if(appversionlist_name == file.value){
                ...
                $.ajax(
                    {
                        type: "GET",
                        url: '/admin/appversionlist/get-name-ajax',
    ...
    function checkupdatestatus(file){
        if(file.value == ''){
            update_status = false;
        }else{
            update_status = true;
    ...
    function checkupdate_prompt(file){
        var update_prompt_reg = /^\d{4}(\-|\/|\.)\d{1,2}\1\d{1,2}$/;
        if(!update_prompt_reg.test(file.value)){
            update_prompt = false;
        }else{
            update_prompt = true;
        }
