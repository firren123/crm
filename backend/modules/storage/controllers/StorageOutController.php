...
class StorageOutController extends BaseController
{
                ...
                if ($info['result'] == 1) {
                    ...
                    if (!empty($info_data)) {
                        foreach ($info_data as $v) {
                            ...
                            //日志
                            $goods = "商品出库关联商品>商品ID:".$v['good_id'].",关联出库单ID:".$v['out_id'].",商品名称:".$v['good_name'].",商品单价:".$v['price'].",商品数量:".$v['num'];
                            $log_model->recordLog($goods, 8);
                        }
                    }

                    //日志
                    $content = "出库订单表>出库原因:".$data['status'].",出库订单编号：".$data['code'].",出库说明:".$data['remark'];
    ...
    public function actionGoodAdd()
    {
        ...
        $files = "id,good_id,sum(num) as total,good_name,attr_value,bar_code";
    ...
    public function actionList()
    {
        ...
        $files = "id,code,create_time,remark,status";
