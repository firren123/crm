...
<div class="wide form">
    <form id="search-form" class="well form-inline" action="/social/orderchinaepay/index" method="get">
            ...
            <table  class="table table-bordered table-hover">
                ...
                <?php if(empty($data)) {
                    ...
                    foreach($data as $item){
                        ?>
                        <tr >
                            <td colspan="2"><a href="<?= '/social/orderchinaepay/detail?order_sn='.$item['order_sn'];?>"><?= $item['order_sn'];?></a></td>
                            <td colspan="2"><a href="/social/user/index?Search[mobile]=<?= $item['mobile']?>" target="_blank"><?= $item['mobile'];?></a></td>
                            <td colspan="2"><?php if(isset($business_type_data[$item['business_type']])){echo $business_type_data[$item['business_type']];}else{echo '未知'.$item['business_type'];};?></td>
                            <td colspan="2"><?= $item['total'];?></td>
                            <td colspan="2"><?= $item['business_code'];?></td>
                            ...
                            <td colspan="2">
                                <?php
                                if(isset($pay_site_list[$item['pay_site_id']])){
                                    echo $pay_site_list[$item['pay_site_id']];
                                }else{
                                    echo '未知';};
                                ?></td>

                            <td colspan="2"><?= $item['create_time'];?></td>
                            <td colspan="2"><?= $source_type_data[$item['source_type']];?></td>

                            <td colspan="4">
                                <a href="<?= '/social/orderchinaepay/detail?order_sn='.$item['order_sn'];?>">详情</a>
                            </td>
                        </tr>
                    <?php } }?>
