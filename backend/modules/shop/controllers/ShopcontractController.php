...
class ShopcontractController extends BaseController
{
    ...
    */
     ...
     * @param array $data_info     xx
     * @param array $ShopManageMsg xx
     * @return null
     */
    private function _addOa($data_info,$ShopManageMsg)
    {
        ...
        $info['data_76'] = CommonHelper::utf8ToGbk($data_info['shop_contract_name']);  //测试注册名称
        $info['data_75'] = CommonHelper::utf8ToGbk($data_info['registered_address']);  //测试注册地址
        $info['data_74'] = CommonHelper::utf8ToGbk($data_info['registered_id']); //测试注册登记号
        $info['data_73'] = CommonHelper::utf8ToGbk($data_info['registered_capital']); //测试注册资本
        $info['data_90'] = CommonHelper::utf8ToGbk($data_info['htnumber']);  //测试合同号
        $info['data_91'] = CommonHelper::utf8ToGbk($this->company_nature_data[$data_info['company_nature']]);  //测试公司性质
        $info['data_72'] = CommonHelper::utf8ToGbk($data_info['legal_representative']);   //测试法定代表人
        $info['data_71'] = CommonHelper::utf8ToGbk($data_info['email']);   //测试邮箱
        $info['data_70'] = CommonHelper::utf8ToGbk($this->document_type_data[$data_info['document_type']]);   //测试证件类型
        $info['data_69'] = CommonHelper::utf8ToGbk($data_info['document_number']); //测试证件号
        ...
        $info['data_58'] = CommonHelper::utf8ToGbk($data_info['common_contacts_phone']);  //测试联系电话
        $info['data_54'] = CommonHelper::utf8ToGbk($data_info['monthly_turnover']);  //测试月均营业额
        $info['data_57'] = CommonHelper::utf8ToGbk($data_info['business_hours']); //测试营业时间
        $info['data_56'] = CommonHelper::utf8ToGbk($data_info['area']);  //测试面积
        ...
        $info['data_47'] = CommonHelper::utf8ToGbk($this->settlement_cycle_data[$data_info['settlement_cycle']]);    //测试结算周期
