...
<div>
    ...
    <?php foreach($map_field_name as $key=>$value){ ?>
        <tr>
            ...
            <?php if(in_array($key,$arr_field_not_edit)){ ?>
                <span class="zjs_<?php echo $key; ?>"><?php echo $arr_shop_info[$key]; ?></span>

            <?php }elseif($key=='manage_type'){ ?>
                <select class="zjs_select_manage_type">
                    <option value="0">未选择</option>
                    <?php foreach($arr_all_manage_type_list as $value){ ?>
                        <option value="<?php echo $value['id']; ?>"<?php if($value['id']==$arr_shop_info['manage_type']){echo "selected=\"selected\"";} ?>><?php echo $value['name']; ?></option>
                    <?php } ?>
                </select>

            <?php }elseif($key=='status'){ ?>
                <select class="zjs_select_status">
                    <?php foreach($map_status as $key=>$value){ ?>
                        <option value="<?php echo $key; ?>"<?php if($key==$arr_shop_info['status']){echo "selected=\"selected\"";} ?>><?php echo $value; ?></option>
                    <?php } ?>
                </select>
            ...
            <?php }elseif($key=='takepoint'){ ?>
                <select class="zjs_select_takepoint">
                    <?php foreach($map_take_point as $key=>$value){ ?>
                        <option value="<?php echo $key; ?>"<?php if($key==$arr_shop_info['takepoint']){echo "selected=\"selected\"";} ?>><?php echo $value; ?></option>
                    <?php } ?>
                </select>
            ...
            <?php }elseif($key=='business_status'){ ?>
                <select class="zjs_select_business_status">
                    <?php foreach($map_business_status as $key=>$value){ ?>
                        <option value="<?php echo $key; ?>"<?php if($key==$arr_shop_info['business_status']){echo "selected=\"selected\"";} ?>><?php echo $value; ?></option>
                    <?php } ?>
                </select>

            <?php }elseif($key=='branch_id'){ ?>
                <select class="zjs_select_branch_open_v4">
                    <?php if(sizeof($arr_all_branch_list)==0){ ?>
                        <option value="0" class="zjs_default_v">请选择分公司</option>
                    <?php } ?>
                    <?php foreach($arr_all_branch_list as $value){ ?>
                        <option value="<?php echo $value['id']; ?>"<?php if($value['id']==$arr_shop_info['branch_id']){echo " selected=\"selected\" ";} ?>><?php echo $value['name']; ?></option>
                    <?php } ?>
                </select>

            <?php }elseif($key=='province'){ ?>
                <select class="zjs_select_province_open_v4">
                    <?php if(sizeof($arr_all_province_list)==0){ ?>
                        <option value="0">请选择省</option>
                    <?php } ?>
                    <?php foreach($arr_all_province_list as $a_provice){ ?>
                        <option value="<?php echo $a_provice['id']; ?>"<?php if($a_provice['id']==$arr_shop_info['province']){echo "selected=\"selected\"";} ?>><?php echo $a_provice['name']; ?></option>
                    <?php } ?>
                </select>

            <?php }elseif($key=='city'){ ?>
                <select class="zjs_select_city_v4">
                    <?php if(sizeof($arr_cur_city_list)==0){ ?>
                        <option value="0" class="zjs_default_v">请选择城市</option>
                    <?php } ?>
                    <?php foreach($arr_cur_city_list as $a_city){ ?>
                        <option value="<?php echo $a_city['id']; ?>"<?php if($a_city['id']==$arr_shop_info['city']){echo " selected=\"selected\" ";} ?>><?php echo $a_city['name']; ?></option>
                    <?php } ?>
                </select>

            <?php }elseif($key=='district'){ ?>
                <select class="zjs_select_district_v4">
                    <?php if(sizeof($arr_cur_district_list)==0){ ?>
                        <option value="0" class="zjs_default_v">请选择区/县</option>
                    <?php } ?>
                    <?php foreach($arr_cur_district_list as $a_district){ ?>
                        <option value="<?php echo $a_district['id']; ?>"<?php if($a_district['id']==$arr_shop_info['district']){echo " selected=\"selected\" ";} ?>><?php echo $a_district['name']; ?></option>
                    <?php } ?>
                </select>
            ...
            <?php }elseif($key=='logo'){ ?>
                ...
                <div class="zjs_area_upload_new_img">
                    <img src="<?php echo \Yii::$app->params['imgHost'].$arr_shop_info['logo']; ?>" class="zjs_logo_full_url" style="max-height:100px;max-width:100px;" />
                    <input type="hidden" class="zjs_logo" value="<?php echo $arr_shop_info['logo']; ?>" />
                </div>
            ...
            <?php }elseif($key=='business_id'){ ?>
                ...
                <?php if(isset($arr_cur_business['id']) && isset($arr_cur_business['name'])){ ?>
                    <option value="<?php echo $arr_cur_business['id']; ?>"><?php echo $arr_cur_business['name']; ?></option>
                <?php }else{ ?>
                    <option value="0">请先进行查询</option>
                <?php } ?>
                </select>
            <?php }elseif($key=='password'){ ?>
                <input type="text" class="zjs_password" value="" style="width:200px;" />
                <span>(如保持空则密码不变，如输入则是设置新密码)</span>
            ...
            <?php $arr_hours=get_hours($arr_shop_info['hours']); ?>
                <select class="zjs_hours_start">
                    <option value="<?php echo $arr_hours['start']; ?>"><?php echo $arr_hours['start']; ?></option>
                </select>
                时~
                <select class="zjs_hours_end">
                    <option value="<?php echo $arr_hours['end']; ?>"><?php echo $arr_hours['end']; ?></option>
                </select>
                时

            <?php }elseif($key=='is_i500'){ ?>
                <select class="zjs_<?php echo $key; ?>">
                    <option value="0" <?php if(isset($arr_shop_info['is_i500']) && $arr_shop_info['is_i500']=='0'){echo " selected=\"selected\" ";} ?>>否</option>
                    <option value="1" <?php if(isset($arr_shop_info['is_i500']) && $arr_shop_info['is_i500']=='1'){echo " selected=\"selected\" ";} ?>>是</option>
                </select>
            <?php }elseif($key=='bank_deposit'){ ?>
                <select class="zjs_bank_deposit">
                    <?php foreach($arr_bank as $value){ ?>
                        <option value="<?php echo $value ?>" <?php if(isset($arr_shop_info['bank_deposit']) && $arr_shop_info['bank_deposit']==$value){echo " selected=\"selected\" ";} ?>><?php echo $value ?></option>
                    <?php } ?>
                </select>
                <input type="text" class="zjs_bank_deposit_branch" style="width:460px;" value="<?php if(isset($arr_shop_info['bank_deposit_branch'])){echo $arr_shop_info['bank_deposit_branch'];} ?>" />
            <?php }elseif($key=='xxxx'){ ?>

            <?php }else{ ?>
                <input type="text" class="zjs_<?php echo $key; ?>" value="<?php if(isset($arr_shop_info[$key])){echo $arr_shop_info[$key];} ?>" />
            <?php } ?>
            </td>
        </tr>
    <?php } ?>
...
<div class="zcss_class0401">
    ...
    <input type="button" class="zjs_btn_submit" value="确认修改" />
</div>
...
function edit()
{
    ...
    var status=$(".zjs_select_status").val();
    ...
    if(isNaN(status)===true){alert("未选择状态");z_log("status not num");return;}
