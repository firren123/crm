...
class Uploader
{
    private $domain = 'http://img1.test.i500m.com/';
    private $server;
    private $storage;
...
//        "POST" => "文件大小超出 post_max_size 限制" ,
    ...
    public function __construct( $fileField , $config , $base64 = false )
    {
        ...
        if(! $this->server){
            Bingo_Log::warning("errno1: " . fastdfs_get_last_error_no() . ", error info: " . fastdfs_get_last_error_info());
            exit(1);
        }
    ...
    private function upFile( $base64 )
    {
        ...
        $up = $this->fdfs_upload($this->fileField);

        $this->fullName = $this->domain . $up['group_name'] . '/' . $up['filename'];
